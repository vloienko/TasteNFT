...
export const InternalModalClosedFirst = ({ active, setActive, children, onClose, removeConnectClass }) => {

   return (
      <div className={active ? "modal__content internal active" : "modal__content internal"} onClick={(e) => e.stopPropagation()}>
         <button className="modal__back" type="button" onClick={() => { onClose(); setActive(false); removeConnectClass(); }}>
            <img src={IconBack} alt="Icon back" />
         </button>
         <button className="modal__closed" type="button" onClick={() => { onClose(); setActive(false); removeConnectClass(); }}>
            <img src={IconClosed} alt="Icon closed" />
         </button>
         {children}
         {/* Вміст модального вікна */}
